...
(bcrypt = require("bcrypt")), (bodyParser = require("body-parser"));
...
//configure mongoose
mongoose.connect(
  process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/Assignment8",
  {
...
app.get("/", (req, res) => {
  res.send("Welcome to Web Design Assignment ");
});
    ...
    if (user) {
      ...
      if (checkPassword(req.body.password)) {
        ...
        res
          ...
          .send({
            message:
              "Please enter a strong password, It should contain 1 samll,1 capital, 1 digit and 1 special character in it",
          });
      }
    ...
    if (passCompare) {
      if (
        ...
        res
          ...
          .send({
            message:
              "Please provide either new name or new password parameters only!",
          });
      } else if (
        req.body.new_name != undefined &&
        req.body.new_password == undefined
        ...
        if (validateName(req.body.new_name)) {
          ...
          res
            .status(400)
            .send({ message: "Please enter the new Name correctly!" });
        }
      } else if (
        req.body.new_name == undefined &&
        req.body.new_password != undefined
        ...
        res
          ...
          .send({
            message:
              "Please provide either the new name or new password, Cannot update email!",
          });
      }
    } else {
      res.status(404).send({
  ...
  User.findOne({ email }).then((user) => {
    ...
    bcrypt.compare(password, user.password, (err, data) => {
      ...
      if (data) {
        ...
        return res
          .status(401)
          .json({ msg: "Invalid credentials!", status: "not ok" });
      }
    });
  });
});
